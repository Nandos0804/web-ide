...
let microphoneDebugIntervalId: number | undefined;

const cleanupMicrophoneBridge = (): void => {
    if (microphoneDebugIntervalId !== undefined) {
        clearInterval(microphoneDebugIntervalId);
        microphoneDebugIntervalId = undefined;
    }
...
const connectMicrophoneToCsoundNode = async (
    ...
    try {
        ...
        const audioInputs = devices
            ...
            .map((device) => ({
                ...
                label: device.label || "(hidden until permission granted)"
            }));
        console.info("Csound Mic Bridge: Available input devices", audioInputs);
    } catch (error) {
        console.warn("Csound Mic Bridge: enumerateDevices failed", error);
    }
    ...
    microphoneProcessingNodes.push(analyser);

    const data = new Float32Array(analyser.fftSize);
    microphoneDebugIntervalId = window.setInterval(() => {
        ...
        console.info(
            "Csound Mic Bridge: Mic level (pre-csound)",
            max.toFixed(4)
        );
    }, 500);
    ...
    if (requestedInputChannels <= 1) {
        ...
        console.info("Csound Mic Bridge: Direct Mono Connection", {
        ...
        console.info("Csound Mic Bridge: Connected (Multi-channel)", {
            requested: requestedInputChannels,
            available: sourceChannels
        });
    }

    console.info("Csound Mic Bridge: Final routing", {
        sourceChannels,
        requestedInputChannels,
        csoundInputs: csoundNode.numberOfInputs
    });
};
...
const parseInputNameFromCsOptions = (csd: string = ""): string | undefined => {
    const match = csd.match(/<CsOptions>[\s\S]*?<\/CsOptions>/i);
    if (!match) return undefined;

    const options = match[0].replace(/;.*$/gm, "");
    const inputMatch =
        options.match(/-i\s*(\S+)/i) || options.match(/--input\s*(\S+)/i);

    return inputMatch ? inputMatch[1].replace(/^['"]|['"]$/g, "") : undefined;
};

const isNativeAdcRequested = (csd: string = ""): boolean => {
    const inputName = parseInputNameFromCsOptions(csd);
    return inputName?.toLowerCase() === "adc";
};

const hasIadcTypoInCsOptions = (csd: string = ""): boolean => {
    const match = csd.match(/<CsOptions>[\s\S]*?<\/CsOptions>/i);
    if (!match) return false;

    const options = match[0].replace(/;.*$/gm, " ");
    return /(?:^|\s)-idac(?:\s|$)/i.test(options);
};

const parseNchnlsInputFromSource = (source: string = ""): number => {
    const match = source.match(/\bnchnls_i\b\s*=\s*(\d+)/i);
...
export const playCsdFromFs = ({
    ...
    return async (dispatch: AppThunkDispatch, setConsole: any) => {
        ...
        const nativeAdcRequested = isNativeAdcRequested(csdContent);
        const iadcTypoDetected = hasIadcTypoInCsOptions(csdContent);
        const requestedInputChannels = parseNchnlsInputFromSource(csdContent);
        const useWorker = localStorage.getItem("sab") === "true";

        if ((nativeAdcRequested || iadcTypoDetected) && !useWorker) {
            dispatch(
                openSnackbar(
                    "Input warning: remove -iadc/-idac from CsOptions when using browser mic bridge.",
        ...
        const csoundObj = await Csound({
            useWorker,
            inputChannelCount: requestedInputChannels
        });
        ...
        installDeterministicMicrophoneBridge(
            csoundObj,
            requestedInputChannels,
            useWorker
        );
        ...
        if (csoundObj) {
            const outputFromCsOptions =
                parseOutputNameFromCsOptions(csdContent);
